...
- [x] `P02.S04` - Report the resolution failure class and the discovery source and resolved port instead of an empty-bodied transport error; `src/vaultspec_rag/serviceclient/_transport.py`.
- [x] `P02.S05` - Fail fast on an absent service with the start-the-service remediation as an isError tool result; `src/vaultspec_rag/mcp/_tools.py`.
...
- [x] `P03.S06` - Remove the admin and lifecycle tools from the MCP surface and stop registering them; `src/vaultspec_rag/mcp/_admin_tools.py`.
- [x] `P03.S07` - Remove the duplicate get_index_status tool; `src/vaultspec_rag/mcp/_tools.py`.
...
- [x] `P04.S08` - Add tool annotations and display titles to the surviving search and refresh and retrieval tools; `src/vaultspec_rag/mcp/_tools.py`.
- [x] `P04.S09` - Add outputSchema and structuredContent and a stable return shape to the search tools; `src/vaultspec_rag/mcp/_tools.py`.
- [x] `P04.S10` - Align the MCP search default result count with the CLI default; `src/vaultspec_rag/mcp/_tools.py`.
...
- [x] `P05.S11` - Treat a stale or orphaned machine pointer as absence and isolate the leaked test token under the managed-storage isolation discipline; `src/vaultspec_rag/_machine_lock.py`.
