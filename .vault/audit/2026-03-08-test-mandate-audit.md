...
1. **`inspect.getsource()` pattern matching** (test_adr_regression.py:193-210, 236-243, 276-293)
...
1. **`isinstance()` checks on Qdrant Filter objects** (test_store.py:25, 37, 62)
...
1. **`threading.Lock()` type checks** (test_adr_regression.py:181-188, 222-229)
...
1. **Manual review:** Read all 27 test files to verify:
...
1. **Consistency check:** Verified all tests follow pytest conventions
