...
- **Pre-Filtering**: Already supported structurally by the metadata schema. `src/vaultspec_rag/store.py` registers `PayloadSchemaType.KEYWORD` indexes for `path`, `language`, `function_name`, `class_name`, and `node_type`, plus an `INTEGER` index for `line_start`. Qdrant can utilize these payload attributes natively to heavily filter the search space prior to executing the expensive RRF evaluation. To support glob matching (`include_paths` / `exclude_paths`), we will map Python `fnmatch` wildcard strings to Qdrant's `MatchText` or `MatchValue` filters, or translate them into regular expressions natively parsed by Qdrant `MatchPattern` filters, thereby avoiding costly post-query Python filtering.
