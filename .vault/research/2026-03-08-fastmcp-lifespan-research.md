...
1. `asyncio.Semaphore(1)` serializes GPU access across concurrent requests
1. Each tool calls `get_comp()` from worker thread (safe due to lock)
...
1. **Startup failure should be fatal** (e.g., required credential missing)
1. **VRAM limits prevent keeping components in memory** (e.g., can't fit model + request buffer)
1. **Server has other services that need initialization** (e.g., database connections)
